...
2. The reporting process shall allow for meaningful communication of the audit
    ...
    * Significant findings shall be reported immediately in a written format.
      CloudCore's security incident response form may be utilized to report a
      single event.
...
5. All CloudCore IT infrastructure must have system clock synchronized
...
CloudCore's Security Officer is authorized to select and use assessment tools
